...
{
    public class WelcomeController(IConnectionMultiplexer redis, TianxingDomainService tianxingDomainService) : BaseController(redis)
    {
        private readonly TianxingDomainService _tianxingDomainService = tianxingDomainService;
        #region 访问
        [HttpGet]
        public async Task<VisitorRes> QueryVisitorCount()
        {
            var db = _redis.GetDatabase();

            var tpvTask = db.StringGetAsync(RedisKeyNames.VisitorToday);
            var apvTask = db.StringGetAsync(RedisKeyNames.Visitor);
            var tuvTask = db.HashLengthAsync(RedisKeyNames.VisitorUserToday);
            var auvTask = db.HashLengthAsync(RedisKeyNames.VisitorUser);

            await Task.WhenAll(tpvTask, apvTask, tuvTask, auvTask);
            return new VisitorRes()
            {
                TodayPv = tpvTask.Result.ToInt(),
                AllPv = apvTask.Result.ToInt(),
                TodayUv = tuvTask.Result.ToInt(),
                AllUv = auvTask.Result.ToInt(),
            };
        }
        ...
        public async Task Visitor()
        {
            ...
            var diff = DateTime.Today.AddDays(1) - DateTime.Now;
            bool todayExists;
            if (!string.IsNullOrEmpty(ip))
            {
                await db.HashSetAsync(RedisKeyNames.VisitorUser, ip, 1);
                todayExists = await db.KeyExistsAsync(RedisKeyNames.VisitorUserToday);
                await db.HashSetAsync(RedisKeyNames.VisitorUserToday, ip, 1);
                if (!todayExists)
                    await db.KeyExpireAsync(RedisKeyNames.VisitorUserToday, diff);
            }
            await db.StringIncrementAsync(RedisKeyNames.Visitor, 1);
            todayExists = await db.KeyExistsAsync(RedisKeyNames.VisitorToday);
            await db.StringIncrementAsync(RedisKeyNames.VisitorToday, 1);
            if (!todayExists)
                await db.KeyExpireAsync(RedisKeyNames.VisitorToday, diff);
        }
        #endregion

        #region 摄像头
        [HttpGet]
        public async Task<List<CameraModel>?> QueryCameras()
        {
            var db = _redis.GetDatabase();

            var list = await db.HashGetAsync<List<CameraModel>>(RedisKeyNames.InitData, CameraDomainService.NAME);
            if (list is { Count: > 0 })
            {
                list = GetRandomIndex(list.Count).Select(s => list[s]).ToList();
            }
            return list;
        }

        private static int[] GetRandomIndex(int arrlen, int take = 5)
        {
            Random rand = new();
            var rt = new int[take];
            ...
            {
                var index = 0;
                while (index == 0 || rt.Any(s => s == index))
                {
                    index = rand.Next(arrlen);
                }
                rt[i] = index;
            }
            return rt;
        ...
        #region 节假日
        [HttpGet]
        public async Task<List<HolidayItemModel>?> QueryHoliday(string date)
        {
            if (!DateTime.TryParseExact(date, "yyyy-MM", CultureInfo.InvariantCulture, DateTimeStyles.None, out var month))
                return null;

            var db = _redis.GetDatabase();

            var holidays = await db.HashGetAsync<List<HolidayItemModel>>(RedisKeyNames.Holiday, date);
            if (holidays is not { Count: > 0 })
            {
                holidays = await _tianxingDomainService.GetHoliday(month);
            }
            return holidays;
        }
        #endregion
        ...
        #region 诗句
        [HttpGet]
        public async Task<PoetryModel?> QueryPoetry()
        {
            var db = _redis.GetDatabase();
            var now = DateTime.Now.ToString("yyyy-MM-dd");
            var poetrys = await db.HashGetAsync<List<PoetryModel>>(RedisKeyNames.Poetry, now);

            if (poetrys is not { Count: > 0 })
                return null;

            var index = GetRandomIndex(poetrys.Count, 1);
            return poetrys[index[0]];
        }
        #endregion

        #region 每日一句
        [HttpGet]
        public async Task<string?> QuerySentence()
        {
            var db = _redis.GetDatabase();
            var now = DateTime.Now.ToString("yyyy-MM-dd");
            var sentences = await db.HashGetAsync<List<string>>(RedisKeyNames.Sentence, now);

            if (sentences is not { Count: > 0 })
                return null;

            var index = GetRandomIndex(sentences.Count, 1);
        ...
        public async Task<List<WordRateModel>?> QueryWordCloud()
        {
            ...
            foreach (var title in titles)
            {
                ...
                foreach (var word in words)
                {
                    ...
                    dic[word] = dic.GetValueOrDefault(word) + 1;
                }
            }

            return dic.Select(s => new WordRateModel { Rate = s.Value, Word = s.Key })
                .OrderByDescending(s => s.Rate).Take(100).ToList();
        }
        #endregion
    }
}
